...
- **Doxygen / CI docs job:** `header.html` no longer uses **`PROJECT_ICON`** / **`COPY_CLIPBOARD`** blocks (unsupported on older Doxygen). **`Doxyfile.in`** lists linked repo Markdown in **`INPUT`** (including **`.cursor/.../SKILL.md`**), **`HAVE_DOT = NO`** avoids missing **`dot`**, README tagline is plain bold (heading nesting warning).
